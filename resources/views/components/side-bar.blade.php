...
<div {{ $attributes->class('md:w-1/4 my-10 pr-6') }} x-transition>
    ...
    <input id="search"
           ...
           class="w-full rounded-md border border-gray-300 shadow-sm px-2 py-2 bg-white text-gray-700 hover:bg-gray-50 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-offset-gray-100 focus:ring-primary-500 dark:bg-slate-900 dark:border-slate-600 dark:focus:ring-offset-slate-700 dark:text-gray-100"
    >
    ...
    @if($ignoredUrls)
        <div class="flex items-center justify-between" x-data="{ 'showIgnoredIssues': @entangle('showIgnoredIssues').live }">
            ...
            <button type="button" x-on:click="showIgnoredIssues = ! showIgnoredIssues" :class="showIgnoredIssues ? 'bg-primary-500' : 'bg-gray-200'" class="relative inline-flex flex-shrink-0 h-6 w-11 border-2 border-transparent rounded-full cursor-pointer transition-colors ease-in-out duration-200 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-primary-500" role="switch" :aria-checked="showIgnoredIssues" aria-labelledby="show-ignored-issues">
    ...
    <div class="hidden md:block">
        ...
        <div x-data="{showMore: false}" class="overflow-hidden relative" :class="showMore ? '' : 'max-h-32'">
            @foreach($repos as $repo)
                <div>
                    <a href="https://github.com/{{ $repo->owner }}/{{ $repo->name }}" class="inline-block items-center px-3 py-1 my-0.5 rounded text-xs font-bold bg-primary-400 dark:bg-primary-600 text-white hover:bg-primary-500 dark:hover:bg-primary-700 transition ease-out">
            ...
            <div x-on:click="showMore = ! showMore" :class="showMore ? '' : 'bg-gradient-to-t from-gray-100 dark:from-slate-700 to-transparent absolute left-0 bottom-0'" class="font-bold h-20 w-full z-10 text-center pt-14 hover:text-gray-500 cursor-pointer">
                <span x-text="showMore ? 'View less' : 'View more'" class="border-b border-b-primary-400">View more</span>
            </div>
        </div>
        ...
        @foreach($labels as $name => $count)
            <div>
                <button
                    @class([
                        'inline-block items-center px-3 py-1 my-0.5 rounded text-xs space-x-1 font-bold text-white hover:bg-primary-500 dark:hover:bg-primary-700 transition ease-out',
                        'bg-primary-400 dark:bg-primary-600' => in_array($name, $this->searchLabels, strict: true),
