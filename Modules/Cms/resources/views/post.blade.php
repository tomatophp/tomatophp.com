...
@section('title', appTitle($post->title))
@section('description', appDescription($post->short_description))
@section('keywords', appKeywords($post->keywords))
    ...
    <div class="bg-slate-50 dark:bg-inherit min-h-screen">
        ...
        <div class="flex flex-col sm:flex-row sm:justify-center gap-2">
            <div class="flex flex-col justify-center sm:justify-end items-center sm:items-end">
                @livewire(\Modules\Cms\Livewire\LikePost::class, ['post' => $post])
            </div>

            <x-cms-social-share />
        </div>

        @livewire(\Modules\Cms\Livewire\CommentPost::class, ['post' => $post])
    </div>
@endsection
